...
# Global varibales
# ---------------------------
# Name rasters for gif
mntit <- c("NTL_Jan22","NTL_Feb22","NTL_Mar22","NTL_Apr22","NTL_May22")
# For ColorCombos382 palette
ntlcol <- c("#09090c", "#19192f",'#1212b8' ,'#f39C12',"#ffb300",'#FFC300',"#ffe200")
# cuts
cuts=c(0,2,5,10,20,50,100,500,30000)
```
...
# Path for rasters mosaics for Ukraine
# -----------------
pthuk22 <- "/Users/parthkhare/Desktop/CGD/Projects/Ukraine FB Mobility NTL/nightlights/Ukraine mosaic_65535rem/2022"
alras22 <- list.files(pthuk22, pattern = ".*tif", full.names = T)

# Read Rasters
# -----------------
rjan22 <-  raster(alras22[3])
proj4string(rjan22) <- CRS("+init=epsg:4326")
rfeb22 <-  raster(alras22[2])
proj4string(rfeb22) <- CRS("+init=epsg:4326")
rmar22 <-  raster(alras22[6])
proj4string(rmar22) <- CRS("+init=epsg:4326")
rapr22 <-  raster(alras22[1])
proj4string(rapr22) <- CRS("+init=epsg:4326")
rmay22 <-  raster(alras22[7])
proj4string(rmay22) <- CRS("+init=epsg:4326")
rjun22 <-  raster(alras22[5])
proj4string(rjun22) <- CRS("+init=epsg:4326")
rjul22 <-  raster(alras22[4])
proj4string(rjul22) <- CRS("+init=epsg:4326")

# Plot January raster 
# -----------------
levelplot(rjan22, at=cuts, par.settings=rasterTheme(ntlcol),main='Before Invasion: Jan 2022',
...
# Plot May raster 
# -----------------
levelplot(rmay22, at=cuts, par.settings=rasterTheme(ntlcol),main='After Invasion: May 2022',
          sub=expression(Watt/cm2/sr),pretty=T,maxpixels = 8222466, margin=F)
# mapView(rmay, col.regions = ntlcol, at=cuts, maxpixels=8222466, alpha.regions = 0.9,zoom=6)

# Plot July raster 
# -----------------
levelplot(rjul22, at=cuts, par.settings=rasterTheme(ntlcol),main='After Invasion: July 2022',
          sub=expression(Watt/cm2/sr),pretty=T,maxpixels = 8222466, margin=F)
...
# Read rasters for Ukraine 2021
# ---------------------------
pthuk21 <- "/Users/parthkhare/Desktop/CGD/Projects/Ukraine FB Mobility NTL/nightlights/Ukraine mosaic_65535rem/2021"
...
```{r message=F,echo=F,warning=FALSE}
# Method II: leaflet
cuts2=c(0,2,5,10,20,50,100,500,30000)
...
leaflet() %>% addTiles() %>%
  ...
  addRasterImage(rjan22, at=cuts2, colors = ntlcb, opacity = 0.9, group="Jan-2022") %>%
  addRasterImage(rfeb22, at=cuts2, colors = ntlcb, opacity = 0.9, group="Feb-2022") %>%
  addRasterImage(rmar22, at=cuts2, colors = ntlcb, opacity = 0.9, group="Mar-2022") %>%
  addRasterImage(rapr22, at=cuts2, colors = ntlcb, opacity = 0.9, group="Apr-2022") %>%
  addRasterImage(rmay22, at=cuts2, colors = ntlcb, opacity = 0.9, group="May-2022") %>%
  addRasterImage(rjun22, at=cuts2, colors = ntlcb, opacity = 0.9, group="Jun-2022") %>%
  addRasterImage(rjul22, at=cuts2, colors = ntlcb, opacity = 0.9, group="Jul-2022") %>%
  addLayersControl(overlayGroups = c("Jan-2022","Feb-2022","Mar-2022","Apr-2022","May-2022",
                                     "Jun-2022","Jul-2022"),
                   options = layersControlOptions(collapsed = FALSE)) %>% 
  hideGroup(c("Feb-2022", "Mar-2022","Apr-2022","May-2022","Jun-2022","Jul-2022"))  %>%
